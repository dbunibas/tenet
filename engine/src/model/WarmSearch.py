...
class WarmSearch(ISearchStrategy):
    ...
    def findEvidences(self, table, numExamples, evidence):  # Query generation
        ...
        evidences = self.executeQuery(query, table, attrMapping, numExamples, evidence, shuffle=True)
        return evidences, query, attrMapping
    ...
    def executeQuery(self, query, table, attrMapping, maxExamples, evidence, shuffle=False):
        ...
        if evidence.getNumberOfCells() > 10 or len(table.rows) > 50:
            log.debug("ExecuteQuery with POSTGRESQL")
            df = runPostgres(query, table, attrMapping)
        else:
            log.debug("ExecuteQuery in memory")
        ...
        for index, row in result.iterrows():
            ...
            for attr, key in self.pairwise(columns):
                ...
                if attribute.type == Constants.NUMERICAL and int(cellValue) == cellValue: cellValue = int(cellValue)
