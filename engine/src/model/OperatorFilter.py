        ...
        if header.type == Constants.NUMERICAL:
            ...
            if (len(uniqueValues) > 1) and (self.comparator == Constants.OPERATOR_GT):
                ...
                if len(diff) > 0:
                    ...
                    if (
                            maxInDataExceptEvidence < minInEvidence):  # and (minInDataExceptEvidence != maxInDataExceptEvidence)):
            ...
            if (len(uniqueValues) > 1) and (self.comparator == Constants.OPERATOR_LT):
                ...
                if len(diff) > 0:
                    ...
                    minInDataExceptEvidence = min(diff)
                    if (
                            minInDataExceptEvidence > maxInEvidence):  # and (minInDataExceptEvidence != maxInDataExceptEvidence)):
            ...
            if len(
                valuesForAttr) < occurrencesInTable: return False  ## Evidence cannot be bigger than the original table
            self.value = value
            return True
