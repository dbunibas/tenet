...
class Cell:
    ...
    def __eq__(self, other):
        return (self.header.name, self.value, self.pos[0], self.pos[1]) == (
        other.name, other.value, other.pos[0], other.pos[1])
...
class Database:
    ...
    def inferTypes(self):
        for table in self.tables:
            data = table.toDict()
            df = pd.DataFrame.from_dict(data).copy()
            cols = df.columns
