...
class OperatorAggregativeFunction(IOperator):
    ...
    def checkSemantic(self, evidence, database) -> bool:
        ...
        if self.operatorFilter is not None:
            ...
            if len(valuesForAttr) != len(
                valuesForColumnInTable): return False  ## Evidence should be the same as the original table
        ...
        if header.type == Constants.CATEGORICAL and self.function in [Constants.OPERATION_SUM,
                                                                      Constants.OPERATION_AVG]: return False
        valuesForAttr = evidence.getValuesForAttr(self.attribute)
