...
class TestTenetFeverous(unittest.TestCase):
    ...
    def test_feverous_load(self):
        ...
        operations = [Constants.OPERATION_LOOKUP, Constants.OPERATION_COMPARISON, Constants.OPERATION_FILTER,
                      Constants.OPERATION_MIN, Constants.OPERATION_MAX, Constants.OPERATION_COUNT,
                      Constants.OPERATION_SUM, Constants.OPERATION_AVG, Constants.OPERATION_RANKED,
                      Constants.OPERATION_PERCENTAGE]
        comparisons = [Constants.OPERATOR_SAME, Constants.OPERATOR_LT, Constants.OPERATOR_GT]
        languageModel = Constants.LANGUAGE_MODEL_CHAT_GTP
        bestEvidences = 1
        sentencesPerExample = 3
        ...
        for feverousEvidence in feverousEvidences:
            ...
            if wiki_page_name in ["Wilford Hundred", "Arturo Maffei", "Colorado State Highway 92", "Erhard Loretan",
                                  "Eurocup Basketball 2013–14 Last 32 Group L",
                                  "Shire of Isisford"]: continue  ## due to tokens limit for chatGPT
            ...
            try:
                ...
                traceback.print_exc()
            try:
                negativeExamples = tenet.generateNegativeExamples(wiki_page_name, evidence, numEvidence, addRows,
                                                                  rowsToAdd, removeRows, rowsToRemove, strategy,
                                                                  useLM=useLM)
    ...
    def test_adults_load(self):
        ...
        for rowIndex in range(0, len(table.rows)):
            ...
            if str(rowTenet[attrNativeCountry].value).strip() == "Japan":
                evidenceFilterJapan.addRow(
                    [table.getCellByPos(rowIndex, attrAgePos), table.getCellByPos(rowIndex, attrNativeCountry)])
        ...
        refuteInstances = refutesInstancesGenerator.generateInstanceForRefuteBigTables(table, addRows, rowsToAdd,
    ...
    def test_feverous_colab(self):
        db = FeverousDB("../data/feverous_wikiv1.db")
        # examplesDev = loadJsonL('../data/revision/original_dev.jsonl')
        examplesDev = loadJsonL('../data/revision/original_dev_corrected.jsonl')
        feverousDev = getFeverousExamples(examplesDev, db)
        examplesTrain = loadJsonL(
            '../data/revision/feverous_train_challenges_only_tables_evidence_add_filtered_st_v2.jsonl')
