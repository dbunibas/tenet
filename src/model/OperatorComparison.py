...
class OperatorComparison(IOperator):
    ...
    def checkSemantic(self, evidence, database) -> bool:
        ...
        if header.type == Constants.CATEGORICAL and self.comparator == Constants.OPERATOR_SAME and len(uniqueValues) > 1: return False
        if header.type == Constants.NUMERICAL and self.comparator == Constants.OPERATOR_SAME and len(uniqueValues) > 1: return False
        return True
