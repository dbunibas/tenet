...
class RefuteInstancesGenerator:
    ...
    #@timed
    def generateInstanceForRefuteBigTables(self, table, addRows, rowsToAdd, removeRows, rowsToRemove, strategy, evidenceSel, evidences=None):
        ...
        if rowsToAdd:
            ...
            if rowsToAdd == 1:
                refuteInstance = self.addTupleBigTable(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MIN, valuesForColumns)
            elif rowsToAdd > 1:
                refuteInstance = self.addTupleBigTable(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MIN, valuesForColumns)
                refuteInstance = self.addTupleBigTable(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MAX, valuesForColumns)
        ...
        if refuteInstance is not None:
            ...
            start = time.time()
            refuteInstance, _ = self.shuffleInstanceBigTable(refuteInstance, evidenceSel, attrFromEvidences=attrFromEvidences)
        ...
        if refuteInstance is not None:
            ...
            start = time.time()
            refuteInstance = self.removeSameRowsBigTable(table, refuteInstance)  ## check if there are identical rows in both instances
    ...
    def generateInstanceForRefute(self, table, addRows, rowsToAdd, removeRows, rowsToRemove, strategy, evidenceSel):
        ...
        if rowsToAdd:
            valuesForColumns = table.toDict()
            if rowsToAdd == 1:
                refuteInstance = self.addTuple(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MIN, valuesForColumns)
            elif rowsToAdd > 1:
                refuteInstance = self.addTuple(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MIN, valuesForColumns)
                refuteInstance = self.addTuple(refuteInstance, 1, strategy, Constants.STRATEGY_CHANGE_MAX, valuesForColumns)
    ...
    def shuffleInstance(self, table, evidenceSel):
        ...
        for i in range(0, colToShuffle):
            ...
            attempt = 0
            while (self.emptyCol(refuteInstance.getCellsByHeader(randomAttr)) or ((len(pickedAttr) > 1) and (randomAttr in pickedAttr))) and attempt < self.attemptsForCol:
                if evidenceSel is None:
            ...
            while attempt < self.attemptsForCol:
                ...
                if (rowNumber < 50 and differences >= rowNumber/2) or (differences > rowNumber*0.05):  #the half of the values are shuffled or 5% in bigger tables
